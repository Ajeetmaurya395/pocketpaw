...
PawKit is a YAML-based configuration format for publishable Command Center templates. A PawKit bundles everything a domain-specific dashboard needs into a single file: layout, automated workflows, user configuration, bundled skills, and integration requirements.

### What's in a PawKit

| Section | Purpose |
|---------|---------|
| `meta` | Name, author, version, category, description, icon |
| `layout` | Grid columns, sections, and panels (tables, charts, kanban boards, feeds) |
| `workflows` | Scheduled or trigger-based agent tasks that run automatically |
| `user_config` | Fields the user fills in during install (API keys, preferences, domain info) |
| `skills` | Bundled domain knowledge for the agent |
| `integrations` | Required and optional external services |
...
workflows:
  content_scan:
    schedule: "daily 8am"
    instruction: "Check all connected platforms for scheduled and published content. Return structured counts."
...
integrations:
  required: []
  optional: [google-calendar, reddit]
```
...
|------|----------------|
| `metrics_row` | Row of stat cards (counts, percentages) |
| `table` | Sortable data table |
| `kanban` | Drag-and-drop columns |
| `chart` | Line, bar, pie, or area chart |
| `feed` | Scrollable activity feed |
| `calendar` | Month/week calendar view |
| `pipeline` | Stage-based pipeline visualization |
| `markdown` | Rendered markdown content |
| `status_list` | Status indicator list |
| `progress_tracker` | Progress bar with milestones |

### Loading a PawKit

```python
from pathlib import Path
from pocketpaw.deep_work.pawkit import load_pawkit, save_pawkit
...
# Load from string
from pocketpaw.deep_work.pawkit import load_pawkit_from_string
config = load_pawkit_from_string(yaml_string)
```

<Callout type="info">
  PawKit is still early — the schema is at v0.1 and will evolve as Command Centers ship. The core structure (meta, layout, workflows, user_config) is stable, but expect new panel types and workflow triggers to be added over time.
</Callout>
